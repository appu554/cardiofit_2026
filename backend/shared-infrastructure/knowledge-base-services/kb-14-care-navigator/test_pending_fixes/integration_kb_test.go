...
func (s *KBIntegrationTestSuite) TestCreateFromKB3TemporalAlert() {
	...
	if rec.Code == http.StatusCreated {
		...
		if data, ok := resp["data"].(map[string]interface{}); ok {
			s.Assert().Equal("KB_3", data["source"])
...
func (s *KBIntegrationTestSuite) TestCreateFromKB9CareGap() {
	...
	if rec.Code == http.StatusCreated {
		...
		if data, ok := resp["data"].(map[string]interface{}); ok {
			s.Assert().Equal("KB_9", data["source"])
		}
	}
}
...
func (s *KBIntegrationTestSuite) TestCreateFromKB12CarePlan() {
	planReq := map[string]interface{}{
		...
		"activity_id":    "ACT-001",
		"activity_type":  "MEDICATION_REVIEW",
		"activity_name":  "Quarterly medication reconciliation",
		"scheduled_date": time.Now().UTC().Add(7 * 24 * time.Hour),
	}
	...
	if rec.Code == http.StatusCreated {
		...
		if data, ok := resp["data"].(map[string]interface{}); ok {
			s.Assert().Equal("KB_12", data["source"])
		}
	}
}
...
func (s *KBIntegrationTestSuite) TestDuplicateKBSourceIDDetected() {
	...
	// Create first task
	task1, _ := s.taskService.CreateTask(s.ctx, &models.CreateTaskRequest{
	...
	// Attempt duplicate
	task2, _ := s.taskService.CreateTask(s.ctx, &models.CreateTaskRequest{
		Type:     models.TaskTypeMonitoringOverdue,
		Source:   models.TaskSourceKB3,
		SourceID: sourceID,
		PatientID: "PATIENT-DUP",
...
func (s *KBIntegrationTestSuite) TestKBSourceHealthCheck() {
	// Check KB-3
	kb3Available := s.kb3Client != nil && s.kb3Client.HealthCheck(s.ctx) == nil

	// Check KB-9
	kb9Available := s.kb9Client != nil && s.kb9Client.HealthCheck(s.ctx) == nil

	// Check KB-12
	kb12Available := s.kb12Client != nil && s.kb12Client.HealthCheck(s.ctx) == nil
...
func (s *KBIntegrationTestSuite) TestEscalationTriggersNotification() {
	...
	// Escalate via service
	escalated, _ := s.taskService.EscalateTask(s.ctx, task.ID, "Test escalation")
	if escalated != nil {
		s.Assert().Equal(models.TaskStatusEscalated, escalated.Status)
	}
}
...
func (s *KBIntegrationTestSuite) createFromCarePlanHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskFactory.CreateFromCarePlan(c.Request.Context(), req)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *KBIntegrationTestSuite) createFromProtocolHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskFactory.CreateFromProtocol(c.Request.Context(), req)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
			return
		}
