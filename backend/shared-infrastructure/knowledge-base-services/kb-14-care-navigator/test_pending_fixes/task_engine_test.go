...
func (s *TaskEngineTestSuite) SetupSuite() {
	...
	// Run migrations
	err = s.db.AutoMigrate(&models.Task{}, &models.Team{}, &models.TeamMember{}, &models.Escalation{})
...
func (s *TaskEngineTestSuite) TestCreateTaskAutoAssignsTaskNumber() {
	task, err := s.taskService.CreateTask(s.ctx, &models.CreateTaskRequest{
...
func (s *TaskEngineTestSuite) TestDuplicateSourceIDConflict() {
	...
	// Create first task
	task1, err := s.taskService.CreateTask(s.ctx, &models.CreateTaskRequest{
		Type:      models.TaskTypeCareGapClosure,
		Source:    models.TaskSourceKB9,
		SourceID:  sourceID,
	...
	// Attempt duplicate
	task2, err := s.taskService.CreateTask(s.ctx, &models.CreateTaskRequest{
		Type:      models.TaskTypeCareGapClosure,
		Source:    models.TaskSourceKB9,
		SourceID:  sourceID,
		PatientID: "PATIENT-003",
...
func (s *TaskEngineTestSuite) TestUpdateTaskSetsUpdatedAt() {
	...
	updated, err := s.taskService.UpdateTask(s.ctx, task.ID, &updateReq)
	s.Require().NoError(err)
...
func (s *TaskEngineTestSuite) TestReassignTaskStatusStaysAssigned() {
	...
	task.Status = models.TaskStatusAssigned
	task.AssignedTo = &originalAssignee
	now := time.Now().UTC()
	task.AssignedAt = &now
	s.db.Save(task)
	...
	s.db.First(&updated, "id = ?", task.ID)
	s.Assert().Equal(models.TaskStatusAssigned, updated.Status)
	s.Assert().Equal(newAssignee, *updated.AssignedTo)
}
...
func (s *TaskEngineTestSuite) TestTaskOptimisticLocking() {
	task := s.createTestTask(models.TaskTypeMedicationReview, models.TaskPriorityMedium)
	originalVersion := task.UpdatedAt

	// First update
	newTitle1 := "Updated Title 1"
	s.db.Model(task).Update("title", newTitle1)
	...
	s.Assert().True(staleTask.UpdatedAt.After(originalVersion))
}
...
func (s *TaskEngineTestSuite) createTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.CreateTask(c.Request.Context(), &req)
		if err != nil {
...
func (s *TaskEngineTestSuite) updateTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.UpdateTask(c.Request.Context(), id, &req)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) assignTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.AssignTask(c.Request.Context(), id, req.AssigneeID, req.Role)
		if err != nil {
...
func (s *TaskEngineTestSuite) startTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.StartTask(c.Request.Context(), id)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) completeTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.CompleteTask(c.Request.Context(), id, req.Outcome, req.Notes)
		if err != nil {
...
func (s *TaskEngineTestSuite) escalateTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.EscalateTask(c.Request.Context(), id, req.Reason)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) declineTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.DeclineTask(c.Request.Context(), id, &req)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) cancelTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.CancelTask(c.Request.Context(), id, &req)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) blockTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.BlockTask(c.Request.Context(), id, req.Reason)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) unblockTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.UnblockTask(c.Request.Context(), id)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *TaskEngineTestSuite) addNoteHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.AddNote(c.Request.Context(), id, req.Content, req.Author, req.AuthorID)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
			return
		}
