...
func (s *AssignmentEscalationTestSuite) setupTestTeams() {
	...
	careTeam := &models.Team{
		...
		Description: "Handles care gaps and chronic care management",
		Active:      true,
		CreatedAt:   time.Now().UTC(),
		UpdatedAt:   time.Now().UTC(),
	}
	s.db.Create(careTeam)
	...
	pharmacyTeam := &models.Team{
		ID:          uuid.New(),
		Name:        "Pharmacy Team",
		Description: "Handles medication reviews and drug interactions",
		Active:      true,
		CreatedAt:   time.Now().UTC(),
		UpdatedAt:   time.Now().UTC(),
	}
	...
	clinicalTeam := &models.Team{
		ID:          uuid.New(),
		Name:        "Clinical Team",
		Description: "Handles critical labs and abnormal results",
		Active:      true,
		CreatedAt:   time.Now().UTC(),
		UpdatedAt:   time.Now().UTC(),
	}
	s.db.Create(clinicalTeam)
	...
	for _, r := range roles {
		for i := 0; i < r.count; i++ {
			member := &models.TeamMember{
				...
				Active:         true,
				CurrentTaskCount: i, // Varying workload
				MaxTaskCount:   10,
				CreatedAt:      time.Now().UTC(),
				UpdatedAt:      time.Now().UTC(),
			}
...
func (s *AssignmentEscalationTestSuite) TestSuggestPrioritizesLeastLoaded() {
	...
	if len(suggestions) > 0 {
		...
		// First suggestion should have lowest current_task_count
		s.Assert().LessOrEqual(int(first["current_task_count"].(float64)), 10)
	}
}
...
func (s *AssignmentEscalationTestSuite) TestAssignTaskSetsAssignedTo() {
	...
	s.Assert().Equal(member.UserID, *updated.AssignedTo)
}
...
func (s *AssignmentEscalationTestSuite) TestAssignTaskSetsAssignedRole() {
	...
	var member models.TeamMember
	s.db.Where("role = ?", "Pharmacist").First(&member)
...
// Test 4.7: Assign increments member's current_task_count
func (s *AssignmentEscalationTestSuite) TestAssignIncrementsWorkload() {
	task := s.createTestTask(models.TaskTypeMedicationReview, models.TaskPriorityHigh)

	var member models.TeamMember
	s.db.First(&member)
	originalCount := member.CurrentTaskCount
	...
	s.Assert().Equal(originalCount+1, updatedMember.CurrentTaskCount)
}
...
func (s *AssignmentEscalationTestSuite) TestReassignDecrementsOldWorkload() {
	...
	s.db.First(&member1)
	s.db.Where("id != ?", member1.ID).First(&member2)

	// First assignment
	task.Status = models.TaskStatusAssigned
	task.AssignedTo = &member1.UserID
	now := time.Now().UTC()
	task.AssignedAt = &now
	s.db.Save(task)

	member1.CurrentTaskCount++
	s.db.Save(&member1)
	originalCount := member1.CurrentTaskCount

	// Reassign
	reassignReq := models.AssignTaskRequest{
		AssigneeID: member2.UserID,
		Role:       member2.Role,
	}
	...
	s.Assert().Equal(originalCount-1, updatedMember1.CurrentTaskCount)
}
...
func (s *AssignmentEscalationTestSuite) TestAssignToInactiveMember400() {
	...
	assignReq := models.AssignTaskRequest{
		AssigneeID: inactiveMember.UserID,
...
func (s *AssignmentEscalationTestSuite) TestAssignAtMaxCapacity400() {
	...
	s.db.Create(maxedMember)
	s.testMembers = append(s.testMembers, maxedMember.ID)

	assignReq := models.AssignTaskRequest{
		AssigneeID: maxedMember.UserID,
...
func (s *AssignmentEscalationTestSuite) TestBulkAssignSuccess() {
	...
	bulkReq := map[string]interface{}{
		"task_ids":    taskIDs,
		"assignee_id": member.UserID.String(),
		"role":        member.Role,
	}
...
func (s *AssignmentEscalationTestSuite) TestAssignmentRespectsTeamBoundaries() {
	...
	for _, suggestion := range suggestions {
		var member models.TeamMember
		s.db.Where("user_id = ?", suggestion.UserID).First(&member)
...
func (s *AssignmentEscalationTestSuite) TestRoundRobinAssignment() {
	...
	for i := 0; i < 6; i++ {
		...
		suggestions, _ := s.assignmentEngine.SuggestAssignees(s.ctx, task.ID)
		if len(suggestions) > 0 {
...
func (s *AssignmentEscalationTestSuite) TestAssignmentWithTeamOverride() {
	...
	assignReq := map[string]interface{}{
		"assignee_id": clinicalMember.UserID.String(),
...
func (s *AssignmentEscalationTestSuite) TestSelfAssignmentAllowed() {
	...
	req := httptest.NewRequest(http.MethodPost,
		fmt.Sprintf("/api/v1/tasks/%s/assign", task.ID.String()), bytes.NewReader(body))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-User-ID", member.UserID.String()) // Self-assign
	rec := httptest.NewRecorder()
	s.router.ServeHTTP(rec, req)
...
func (s *AssignmentEscalationTestSuite) TestAssignmentValidatesRoleCompatibility() {
	...
	// Try to assign critical lab to non-physician
	var nonPhysician models.TeamMember
	s.db.Where("role != ?", "Physician").First(&nonPhysician)

	assignReq := models.AssignTaskRequest{
		AssigneeID: nonPhysician.UserID,
...
func (s *AssignmentEscalationTestSuite) TestEscalationThresholdConfiguration() {
	...
	standardThresholds := models.GetStandardEscalationThresholds()
	criticalThresholds := models.GetCriticalEscalationThresholds()

	// Standard: 50%, 75%, 100%, 125%
	s.Assert().Equal(0.50, standardThresholds.WarningPercent)
	s.Assert().Equal(0.75, standardThresholds.UrgentPercent)
	s.Assert().Equal(1.00, standardThresholds.CriticalPercent)
	s.Assert().Equal(1.25, standardThresholds.ExecutivePercent)

	// Critical: 25%, 50%, 75%, 100%
	s.Assert().Equal(0.25, criticalThresholds.WarningPercent)
	s.Assert().Equal(0.50, criticalThresholds.UrgentPercent)
	s.Assert().Equal(0.75, criticalThresholds.CriticalPercent)
	s.Assert().Equal(1.00, criticalThresholds.ExecutivePercent)
}
...
func (s *AssignmentEscalationTestSuite) suggestAssigneeHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		suggestions, err := s.assignmentEngine.SuggestAssignees(c.Request.Context(), taskID)
		if err != nil {
...
func (s *AssignmentEscalationTestSuite) getWorkloadHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		utilization := float64(member.CurrentTaskCount) / float64(member.MaxTaskCount) * 100

		c.JSON(http.StatusOK, gin.H{
			"current_count": member.CurrentTaskCount,
			"max_count":     member.MaxTaskCount,
...
func (s *AssignmentEscalationTestSuite) assignTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err := s.db.Where("user_id = ?", req.AssigneeID).First(&member).Error; err == nil {
			...
			if member.CurrentTaskCount >= member.MaxTaskCount {
				c.JSON(http.StatusBadRequest, gin.H{"success": false, "error": "assignee at max capacity"})
				return
			}
		}

		task, err := s.taskService.AssignTask(c.Request.Context(), taskID, req.AssigneeID, req.Role)
		if err != nil {
		...
		s.db.Model(&models.TeamMember{}).Where("user_id = ?", req.AssigneeID).
			UpdateColumn("current_task_count", gorm.Expr("current_task_count + 1"))
	...
	return func(c *gin.Context) {
		...
		if oldAssignee != nil {
			s.db.Model(&models.TeamMember{}).Where("user_id = ?", oldAssignee).
				UpdateColumn("current_task_count", gorm.Expr("GREATEST(current_task_count - 1, 0)"))
		}
...
func (s *AssignmentEscalationTestSuite) escalateTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.EscalateTask(c.Request.Context(), taskID, req.Reason)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *AssignmentEscalationTestSuite) checkAllEscalationsHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		escalatedCount, err := s.escalationEngine.CheckAllEscalations(c.Request.Context())
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "error": err.Error()})
			return
		}
...
func (s *AssignmentEscalationTestSuite) completeTaskHandler() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		task, err := s.taskService.CompleteTask(c.Request.Context(), taskID, req.Outcome, req.Notes)
		if err != nil {
			c.JSON(http.StatusConflict, gin.H{"success": false, "error": err.Error()})
			return
		}
