...
func main() {
	...
	// + Shared Context Router (ONC → OHDSI → LOINC Execution Contract)
	logger.Info("Initializing OHDSI Constitutional DDI Service...")
	var ohdsiEnabled bool
	var ohdsiExpansionService *services.OHDSIExpansionService
	var sharedContextRouter *contextrouter.ContextRouter
	...
	if sharedDBErr != nil {
		...
		ohdsiExpansionService = services.NewOHDSIExpansionService(sharedDB, metricsCollector)
		ohdsiEnabled = true
		logger.Info("OHDSI Constitutional DDI Service initialized (25 ONC rules)")

		// Initialize shared Context Router for LOINC evaluation
		// Golden Rule: "Class Expansion NEVER checks LOINC. Context Router ALWAYS does."
		routerConfig := contextrouter.DefaultRouterConfig()
		sharedContextRouter = contextrouter.NewContextRouter(logger, routerConfig)
		logger.Info("Shared Context Router initialized (ONC → OHDSI → LOINC pipeline)")
	}
	...
	// Register Execution Contract handlers (ONC → OHDSI → LOINC pipeline)
	if ohdsiEnabled && ohdsiExpansionService != nil && sharedContextRouter != nil {
		executionHandlers := api.NewExecutionHandlers(ohdsiExpansionService, sharedContextRouter)
		executionHandlers.RegisterRoutes(httpServer.Router.Group("/api/v1"))
		logger.Info("Execution Contract handlers registered",
			zap.String("endpoint", "/api/v1/execution/evaluate"),
			zap.String("contract", "/api/v1/execution/contract"))
	}
...
Execution Contract (ONC → OHDSI → LOINC):
- DDI Evaluation: POST /api/v1/execution/evaluate
- Contract Spec: GET /api/v1/execution/contract
