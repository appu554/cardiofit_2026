...
func seedMatchedPairs(t *testing.T, db *gorm.DB, cohort, intervention string, horizon int, predCATE, attribEffect float64, n int) {
	...
	for i := 0; i < n; i++ {
		rid := uuid.New()
		db.Exec(`INSERT INTO cate_estimates
			...
			predCATE, predCATE-0.03, predCATE+0.03, horizon, string(models.OverlapPass), now)
		db.Exec(`INSERT INTO attribution_verdicts
			...
			attribEffect, horizon, "RULE_BASED", now)
	}
}
