...
//
// This is the KB-23 mirror of the Phase 5 P5-3 abstraction that lives
// in KB-26. Phase 6 P6-5 extracts the same shape into KB-23 so renal
// anticipatory monthly batches and any future KB-23 batch consumers
// can share one scheduler. The interface is intentionally identical to
// KB-26's so a future shared-infrastructure batch package can absorb
// both without semantic drift.
type BatchJob interface {
	...
	// wall-clock time. Lets the scheduler host jobs with different
	// cadences (daily, weekly, monthly, sensor-change) on a single
	// hourly tick without baking any cadence semantics into the
	// scheduler itself. The job owns its own "is it time?" logic.
...
// BatchScheduler runs registered BatchJobs on a generic tick. Phase 6 P6-5
// adds the first KB-23 consumer (RenalAnticipatoryBatch); the interface
// is intentionally generic so future jobs can be added without rewriting
// the scheduler.
...
func (s *BatchScheduler) RunOnce(ctx context.Context) error {
	...
	for _, job := range jobs {
		...
		// Consult the job's own cadence gate. Jobs whose ShouldRun
		// returns false are silently skipped — the scheduler has no
...
// Production wires this to an hourly tick so jobs with hour-aligned
// cadences (daily/monthly via ShouldRun) self-filter correctly.
