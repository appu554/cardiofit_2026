...
type ChannelBProjection struct {
	...
	FinerenoneActive              bool `json:"finerenone_active"` // B-21: finerenone hyperkalemia monitoring
	...
	// ── Glucose variability (B-20: CV% > 36% → PAUSE) ──
	GlucoseCV30d *float64 `json:"glucose_cv_30d"` // from FBGTracking.CV30d
...
type ChannelCProjection struct {
	...
	// ── ACR-RAAS escalation (PG-17) ──
	ACRCategory string `json:"acr_category"` // A1 | A2 | A3 (from ACRTracking)
	ACRA3NoRAAS bool   `json:"acr_a3_no_raas"` // PG-17-A3: ACR A3 + no ACEi/ARB → HALT
	ACRA2NoRAAS bool   `json:"acr_a2_no_raas"` // PG-17-A2: ACR A2 + no ACEi/ARB → MODIFY

	// ── Finerenone eligibility and monitoring (PG-18/PG-19) ──
	FinerenoneEligible    bool `json:"finerenone_eligible"`     // PG-18: eGFR 25-75 + ACR A2/A3 + on ACEi/ARB
	FinerenoneKMonitoring bool `json:"finerenone_k_monitoring"` // PG-19: finerenone active + K+ ≥ 5.0

	// ── Therapeutic futility (PG-06) ──
	CyclesSinceHbA1cImprovement int `json:"cycles_since_hba1c_improvement"` // PG-06: ≥12 → PAUSE
