...
###############################################################################
# KB-7 SNOMED CT-AU Downloader Deployment Script
###############################################################################
# Builds and deploys the Australian NTS downloader as a Cloud Run Job
# Downloads SNOMED CT-AU (Module 32506021000036107) + AMT (Module 900062011000036103)
###############################################################################
...
DOWNLOADER_DIR="$FUNCTIONS_DIR/snomed-au-downloader"
...
IMAGE_NAME="snomed-au-downloader"
IMAGE_TAG="${IMAGE_TAG:-latest}"
FULL_IMAGE="$REGION-docker.pkg.dev/$PROJECT_ID/$REPOSITORY/$IMAGE_NAME:$IMAGE_TAG"

# Cloud Run Job configuration
JOB_NAME="kb7-snomed-au-job-${ENVIRONMENT}"
SOURCE_BUCKET="${PROJECT_ID}-kb-sources"
NTS_SECRET_NAME="kb7-nts-australia-credentials"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}KB-7 SNOMED CT-AU Downloader Deployment${NC}"
echo -e "${BLUE}========================================${NC}\n"
...
if [ ! -d "$DOWNLOADER_DIR" ]; then
    echo -e "${RED}Error: Downloader directory not found: $DOWNLOADER_DIR${NC}"
    exit 1
fi
...
if [ "$SKIP_BUILD" = false ]; then
    ...
    if [ "$DRY_RUN" = true ]; then
        ...
        # Build and push using Cloud Build (faster, no local Docker daemon needed)
        cd "$DOWNLOADER_DIR"
...
if [ "$DRY_RUN" = true ]; then
    echo -e "${CYAN}  [DRY-RUN] Would deploy job: ${JOB_NAME}${NC}"
else
    ...
    gcloud run jobs $ACTION "$JOB_NAME" \
        --region="$REGION" \
        --image="$FULL_IMAGE" \
        --set-env-vars="PROJECT_ID=${PROJECT_ID},SOURCE_BUCKET=${SOURCE_BUCKET},ENVIRONMENT=${ENVIRONMENT},NTS_SECRET_NAME=${NTS_SECRET_NAME},PACKAGE_TYPE=SNOMEDCT-AU RF2 SNAPSHOT" \
        --memory=4Gi \
        --cpu=2 \
        --task-timeout=3600s \
        --max-retries=2 \
        --set-secrets="NTS_CREDENTIALS=${NTS_SECRET_NAME}:latest" \
        --quiet 2>/dev/null || \
    gcloud run jobs $ACTION "$JOB_NAME" \
        --region="$REGION" \
        --image="$FULL_IMAGE" \
...
echo -e "${BLUE}SNOMED CT-AU Downloader:${NC}"
...
echo -e "  - Australian dialect reference sets"
echo -e ""
echo -e "${BLUE}Data Source:${NC}"
echo -e "  Australian Digital Health Agency (ADHA)"
echo -e "  National Terminology Service (NTS)"
echo -e "  https://www.healthterminologies.gov.au"
echo -e ""
