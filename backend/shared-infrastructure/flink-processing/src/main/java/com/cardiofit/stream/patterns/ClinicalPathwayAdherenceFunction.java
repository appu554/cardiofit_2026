...
public class ClinicalPathwayAdherenceFunction
    ...
    private static final String DIAGNOSIS_ADDED = "DIAGNOSIS_ADDED";
    ...
    public void processElement(
        ...
        String patientId = ((CanonicalEvent)event).getPatientId();
        ...
        for (Protocol protocol : applicableProtocols) {
            ...
            if (analysis.hasDeviation()) {
                ...
                if (analysis.getSeverity() == PathwayAnalysis.Severity.CRITICAL) {
    ...
    private PathwayAnalysis analyzePathwayAdherence(
        ...
        Set<String> expectedActions = new java.util.HashSet<>(protocol.getExpectedActions(protocolState.getCurrentStep()));
    ...
    private PatientContext updatePatientContext(PatientEvent event) throws Exception {
        ...
        if (context == null) {
            context = new PatientContext(((CanonicalEvent)event).getPatientId());
        }
        ...
        switch (eventType) {
            case MEDICATION_PRESCRIBED:
                context.addActiveMedication(getMedicationFromEvent(event));
                break;
            case MEDICATION_DISCONTINUED:
                context.removeActiveMedication(getMedicationFromEvent(event));
                break;
            case LAB_RESULT:
                context.updateLabValue(getLabTypeFromEvent(event), getLabValueFromEvent(event));
                break;
            case DIAGNOSIS_ADDED:
                context.addCondition(getDiagnosisFromEvent(event));
                break;
            case VITAL_SIGNS:
                context.updateVitals(getVitalsFromEvent(event));
                break;
    ...
    private Severity calculateDeviationSeverity(
        ...
        if ("MEDICATION_PRESCRIBED".equals(event.getEventType())) {
    ...
    private Alert createDeviationAlert(
        ...
        // Convert PathwayAnalysis.Severity enum to String for Alert
        alert.setSeverity(analysis.getSeverity().name());
        ...
        alert.setRiskScore(analysis.getConfidence());
        alert.setTimestamp(System.currentTimeMillis());
        alert.setRiskScore(patientContext.calculateRiskScore());
    ...
    private EvidenceEnvelope createEvidenceEnvelope(
        ...
        // Convert long timestamp to LocalDateTime for EvidenceEnvelope
        envelope.setTimestamp(java.time.LocalDateTime.ofInstant(
    ...
    // Helper methods to extract data from PatientEvent clinical data
    private String getMedicationFromEvent(PatientEvent event) {
        if (event.getClinicalData() != null) {
            Object medication = event.getClinicalData().get("medication");
            return medication != null ? medication.toString() : "unknown_medication";
        }
        return "unknown_medication";
    }

    private String getLabTypeFromEvent(PatientEvent event) {
        if (event.getClinicalData() != null) {
            Object labType = event.getClinicalData().get("lab_type");
            return labType != null ? labType.toString() : "unknown_lab";
        }
        return "unknown_lab";
    }

    private Object getLabValueFromEvent(PatientEvent event) {
        if (event.getClinicalData() != null) {
            return event.getClinicalData().get("lab_value");
        }
        return 0.0;
    }

    private String getDiagnosisFromEvent(PatientEvent event) {
        if (event.getClinicalData() != null) {
            Object diagnosis = event.getClinicalData().get("diagnosis");
            return diagnosis != null ? diagnosis.toString() : "unknown_diagnosis";
        }
        return "unknown_diagnosis";
    }

    private Map<String, Object> getVitalsFromEvent(PatientEvent event) {
        if (event.getClinicalData() != null) {
