...
public class EnrichedPatientEvent {
    ...
    // Additional methods for Flink mapper compatibility
    public List<DrugInteraction> getDrugInteractions() {
        // Extract drug interactions from clinical insights or return empty list
        return new ArrayList<>(); // TODO: implement proper extraction logic
    }

    public VitalSigns getVitalSigns() {
        // Extract vital signs from patient context or return null
        return null; // TODO: implement proper extraction logic
    }

    public List<AllergyAlert> getAllergyAlerts() {
        // Extract allergy alerts from clinical insights or return empty list
        return new ArrayList<>(); // TODO: implement proper extraction logic
    }

    public List<SafetyViolation> getSafetyViolations() {
        // Extract safety violations from clinical insights or return empty list
        return new ArrayList<>(); // TODO: implement proper extraction logic
    }

    public List<String> getClinicalPatterns() {
        // Extract clinical patterns from detected patterns
        List<String> patterns = new ArrayList<>();
        if (detectedPatterns != null) {
            for (DetectedPattern pattern : detectedPatterns) {
                patterns.add(pattern.getPatternType());
            }
        }
        return patterns;
    }
    ...
    public Double getConfidenceScore() {
        // Calculate overall confidence score from clinical insights
        if (clinicalInsights != null && !clinicalInsights.isEmpty()) {
            double totalConfidence = 0.0;
            for (ClinicalInsight insight : clinicalInsights) {
                if (insight.getConfidence() != null) {
                    totalConfidence += insight.getConfidence();
                }
            }
            return totalConfidence / clinicalInsights.size();
        }
        return 0.0;
    }

    public Map<String, Object> getSemanticMeshData() {
        // Return semantic enrichment data
        return semanticEnrichment != null ? semanticEnrichment : new HashMap<>();
    }

    public PatientContext getPatientContextSnapshot() {
        // Return patient context as snapshot
        return patientContext;
    }
