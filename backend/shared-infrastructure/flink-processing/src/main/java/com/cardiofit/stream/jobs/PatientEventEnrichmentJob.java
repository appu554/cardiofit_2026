...
import com.cardiofit.flink.operators.TransactionalMultiSinkRouter;
import com.cardiofit.flink.models.EnrichedClinicalEvent;
import com.cardiofit.flink.mappers.RoutedEventToEnrichedEventMapper;
 ...
 * 6. Transform to EnrichedClinicalEvent format
 * 7. Route to hybrid Kafka topics via TransactionalMultiSinkRouter
 *
 * Performance Targets:
...
public class PatientEventEnrichmentJob {
    ...
    // Kafka topic configuration - versioned source topics
    ...
    private static void configureExecutionEnvironment(StreamExecutionEnvironment env) {
        ...
        // Flink 2.x: Use Configuration-based restart strategy
    ...
    private static void createProcessingPipeline(StreamExecutionEnvironment env) throws Exception {
        ...
        // 3. Key by patient ID for stateful processing
        // 4. Enrich events with semantic mesh data
        SingleOutputStreamOperator<EnrichedPatientEvent> enrichedEvents = filteredEvents
        ...
        // 6. Transform to EnrichedClinicalEvent format for hybrid routing
        SingleOutputStreamOperator<EnrichedClinicalEvent> transformedEvents = patternDetectedEvents
            .map(new RoutedEventToEnrichedEventMapper())
            .uid("Transform to Enriched Clinical Events")
            .uid("transform-to-enriched");

        logger.info("🔄 Added transformation to EnrichedClinicalEvent format");

        // 7. Route to hybrid Kafka topics using TransactionalMultiSinkRouter
        transformedEvents
            .process(new TransactionalMultiSinkRouter())
            .uid("Transactional Multi-Sink Router")
            .uid("transactional-router");

        logger.info("✅ Processing pipeline created with hybrid topic routing");
    }
    ...
    private static WatermarkStrategy<PatientEvent> createWatermarkStrategy() {
        return WatermarkStrategy
            ...
            .withTimestampAssigner((event, timestamp) -> {
                // Use getTimestampAsDateTime() which returns LocalDateTime, not the primitive long from getTimestamp()
                LocalDateTime eventTime = event.getTimestampAsDateTime();
    ...
    /**
     * NOTE: Multi-sink distribution has been replaced by TransactionalMultiSinkRouter
     *
     * The new hybrid architecture routes events to specialized Kafka topics:
     ...
     * - prod.ehr.analytics.events (High-throughput analytics)
     * - prod.ehr.graph.mutations (Neo4j relationship updates)
     * - prod.ehr.audit.logs (Compliance and audit trails)
     *
     * Downstream systems consume from these topics via Kafka Connect:
     * - FHIR Store: Kafka Connect → Google Healthcare API
     * - Analytics: Kafka Connect → ClickHouse
     * - Notifications: Kafka Connect → Notification Services
     * - Graph DB: Kafka Connect → Neo4j
     * - Cache: Kafka Connect → Redis
     */
