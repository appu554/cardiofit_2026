#!/usr/bin/env python3
"""
Readmission Risk Model Training Pipeline

Complete end-to-end training pipeline for 30-day unplanned readmission prediction using MIMIC-IV data.

Clinical Focus:
- 30-day unplanned readmission prediction
- Length of stay
- Discharge diagnosis complexity, prior admission history
...
Target Metrics:
- AUROC > 0.80
- Sensitivity > 0.70 (balanced prediction)
- Specificity > 0.75
...
class ReadmissionModelTrainer:
    ...
    def optimize_hyperparameters(self, X_train, y_train, n_trials=50):
        ...
        # Run optimization
        study = optuna.create_study(direction='maximize', study_name='readmission_xgboost')
    ...
    def export_to_onnx(self, output_path, version='1.0.0'):
        ...
        onnx_model.doc_string = '30-day unplanned readmission prediction'

        # Add custom metadata
        meta_model_name = onnx_model.metadata_props.add()
        meta_model_name.key = 'model_name'
        meta_model_name.value = 'readmission_risk'
        ...
        meta_clinical.value = 'Length of stay, discharge diagnosis, prior admissions, comorbidities'
...
def main():
    ...
    print("=" * 70)
    print("READMISSION RISK MODEL TRAINING PIPELINE")
    print("=" * 70)
    print(f"\nInput:      {args.input}")
    ...
    try:
        # Initialize trainer
        trainer = ReadmissionModelTrainer(random_state=args.seed)
