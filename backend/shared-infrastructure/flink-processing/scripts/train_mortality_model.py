#!/usr/bin/env python3
"""
Mortality Risk Model Training Pipeline

Complete end-to-end training pipeline for in-hospital mortality prediction using MIMIC-IV data.

Clinical Focus:
- In-hospital mortality prediction
- Age and comorbidity burden
- APACHE II score, organ dysfunction markers, severity of illness
...
Target Metrics:
- AUROC > 0.85
- Sensitivity > 0.75 (balanced sensitivity and specificity)
- Specificity > 0.80
...
class MortalityModelTrainer:
    ...
    def optimize_hyperparameters(self, X_train, y_train, n_trials=50):
        ...
        # Run optimization
        study = optuna.create_study(direction='maximize', study_name='mortality_xgboost')
    ...
    def export_to_onnx(self, output_path, version='1.0.0'):
        ...
        onnx_model.doc_string = 'In-hospital mortality prediction'

        # Add custom metadata
        meta_model_name = onnx_model.metadata_props.add()
        meta_model_name.key = 'model_name'
        meta_model_name.value = 'mortality_risk'
        ...
        meta_clinical.value = 'Age, comorbidities, APACHE, organ dysfunction, bilirubin'
...
def main():
    ...
    print("=" * 70)
    print("MORTALITY RISK MODEL TRAINING PIPELINE")
    print("=" * 70)
    print(f"\nInput:      {args.input}")
    ...
    try:
        # Initialize trainer
        trainer = MortalityModelTrainer(random_state=args.seed)
