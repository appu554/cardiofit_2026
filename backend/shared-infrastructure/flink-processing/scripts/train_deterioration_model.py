#!/usr/bin/env python3
"""
Deterioration Risk Model Training Pipeline

Complete end-to-end training pipeline for clinical deterioration prediction using MIMIC-IV data.

Clinical Focus:
- Clinical deterioration risk (6-24 hour prediction window)
- Vital sign trends (6-hour changes)
- NEWS2 and qSOFA score progression, respiratory distress indicators
...
Target Metrics:
- AUROC > 0.85
- Sensitivity > 0.80 (high recall for deterioration detection)
- Specificity > 0.75
...
class DeteriorationModelTrainer:
    ...
    def optimize_hyperparameters(self, X_train, y_train, n_trials=50):
        ...
        # Run optimization
        study = optuna.create_study(direction='maximize', study_name='deterioration_xgboost')
    ...
    def export_to_onnx(self, output_path, version='1.0.0'):
        ...
        onnx_model.doc_string = 'Clinical deterioration risk (6-24 hour prediction window)'

        # Add custom metadata
        meta_model_name = onnx_model.metadata_props.add()
        meta_model_name.key = 'model_name'
        meta_model_name.value = 'deterioration_risk'
        ...
        meta_clinical.value = 'Vital trends, NEWS2, respiratory rate, MAP, lactate'
...
def main():
    ...
    print("=" * 70)
    print("DETERIORATION RISK MODEL TRAINING PIPELINE")
    print("=" * 70)
    print(f"\nInput:      {args.input}")
    ...
    try:
        # Initialize trainer
        trainer = DeteriorationModelTrainer(random_state=args.seed)
