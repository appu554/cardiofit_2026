...
def get_header_auth_middleware():
    ...
    # Need to go up two levels: app -> patient-service -> services
...
def get_auth_components():
    """
    Get the authentication components using a direct import approach.
    
    Returns:
        A tuple of (HeaderAuthMiddleware, get_current_user, require_permissions, require_role)
    """
    # Get the absolute path to the backend directory
    # Need to go up three levels: app -> patient-service -> services -> backend
    backend_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.."))
    ...
    try:
        ...
        logger.info("Successfully imported auth components from backend/shared/auth")
        return HeaderAuthMiddleware, get_current_user, require_permissions, require_role
    except ImportError as e:
        logger.error(f"Backend import failed: {e}")
        raise ImportError(f"Could not import auth components from backend/shared/auth")

# Try to get the HeaderAuthMiddleware class
try:
    logger.info("Attempting to get HeaderAuthMiddleware...")
    HeaderAuthMiddleware, get_current_user, require_permissions, require_role = get_auth_components()
