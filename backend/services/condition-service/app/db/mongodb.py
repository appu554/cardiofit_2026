...
async def connect_to_mongo(max_retries: int = 3, retry_delay: int = 2) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Ensure the conditions collection exists
            await ensure_collection_exists("conditions")
...
def get_conditions_collection():
    ...
    if not db.is_connected():
        logger.warning("Database not connected, cannot get conditions collection")
        return None

    try:
        # Get the collection
        if db.db is None:
            logger.error("Database object is None, cannot get conditions collection")
            return None

        collection = db.db.conditions
        return collection
    except Exception as e:
        logger.error(f"Error getting conditions collection: {str(e)}")
        return None
