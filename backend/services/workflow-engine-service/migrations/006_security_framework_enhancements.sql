-- Migration: Security Framework Enhancements
...
-- Add security-related columns to existing clinical_audit_trail table
DO $security_enhancements$
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'audit_level_type') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clinical_audit_trail' AND column_name = 'event_type') THEN
        ALTER TABLE clinical_audit_trail ADD COLUMN event_type audit_event_type;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clinical_audit_trail' AND column_name = 'audit_level_enum') THEN
        ALTER TABLE clinical_audit_trail ADD COLUMN audit_level_enum audit_level_type DEFAULT 'standard';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clinical_audit_trail' AND column_name = 'outcome') THEN
        ALTER TABLE clinical_audit_trail ADD COLUMN outcome VARCHAR(50) DEFAULT 'success';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clinical_audit_trail' AND column_name = 'error_details') THEN
        ALTER TABLE clinical_audit_trail ADD COLUMN error_details JSONB DEFAULT '{}';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'clinical_audit_trail' AND column_name = 'safety_critical') THEN
        ALTER TABLE clinical_audit_trail ADD COLUMN safety_critical BOOLEAN DEFAULT FALSE;
    END IF;
END $security_enhancements$;

-- =====================================================
-- BREAK-GLASS ACCESS ENHANCEMENTS
-- =====================================================

-- Enhance emergency_access_records table with break-glass session data
DO $break_glass_enhancements$
BEGIN
    -- Add break-glass session columns if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'session_id') THEN
        ALTER TABLE emergency_access_records ADD COLUMN session_id VARCHAR(255) UNIQUE;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'access_type') THEN
        ALTER TABLE emergency_access_records ADD COLUMN access_type VARCHAR(100);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'justification') THEN
        ALTER TABLE emergency_access_records ADD COLUMN justification VARCHAR(100);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'clinical_details') THEN
        ALTER TABLE emergency_access_records ADD COLUMN clinical_details TEXT;
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'supervisor_approval') THEN
        ALTER TABLE emergency_access_records ADD COLUMN supervisor_approval VARCHAR(255);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'actions_performed') THEN
        ALTER TABLE emergency_access_records ADD COLUMN actions_performed JSONB DEFAULT '[]';
    END IF;

    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'emergency_access_records' AND column_name = 'audit_trail_ids') THEN
        ALTER TABLE emergency_access_records ADD COLUMN audit_trail_ids JSONB DEFAULT '[]';
    END IF;
END $break_glass_enhancements$;
...
CREATE TABLE IF NOT EXISTS phi_access_log (
    ...
    audit_trail_id UUID REFERENCES clinical_audit_trail(id),
    user_id VARCHAR(255) NOT NULL,
    patient_id VARCHAR(255) NOT NULL,
    workflow_instance_id INTEGER REFERENCES workflow_instances(id),
    access_type VARCHAR(100) NOT NULL, -- encrypt, decrypt, view, export
    ...
    ip_address INET,
    user_agent TEXT,
    access_purpose VARCHAR(255), -- clinical_care, emergency, audit, export
    data_classification VARCHAR(50) DEFAULT 'phi', -- phi, sensitive, public
...
CREATE TABLE IF NOT EXISTS security_events (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    event_type VARCHAR(100) NOT NULL, -- failed_login, suspicious_access, data_breach_attempt
    ...
    source_ip INET,
    user_agent TEXT,
    detection_method VARCHAR(100), -- automated, manual, system
...
CREATE TABLE IF NOT EXISTS clinical_decision_audit (
    ...
    decision_type VARCHAR(100) NOT NULL, -- medication_prescription, treatment_plan, discharge_decision
    ...
    evidence_sources JSONB DEFAULT '[]', -- Guidelines, studies, protocols used
...
CREATE TABLE IF NOT EXISTS encrypted_workflow_states (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    workflow_instance_id INTEGER REFERENCES workflow_instances(id) UNIQUE,
    encrypted_state TEXT NOT NULL, -- Base64 encoded encrypted JSON
...
CREATE TRIGGER update_phi_access_log_updated_at 
...
CREATE TRIGGER update_clinical_decision_audit_updated_at 
    BEFORE UPDATE ON clinical_decision_audit 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

CREATE TRIGGER update_encrypted_workflow_states_updated_at 
    BEFORE UPDATE ON encrypted_workflow_states 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
SELECT 
    ...
    COUNT(*) FILTER (WHERE array_length(CAST(overrides_applied AS TEXT[]), 1) > 0) as decisions_with_overrides
