...
async def connect_to_mongo(max_retries: int = 3, retry_delay: int = 2) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Ensure the encounters collection exists
            await ensure_collection_exists("encounters")
...
def get_encounters_collection():
    """Get the encounters collection."""
    # Check connection status
    if not db.is_connected():
        logger.warning("Database not connected, cannot get encounters collection")
        return None

    try:
        # Get the collection
        if db.db is None:
            logger.error("Database object is None, cannot get encounters collection")
            return None

        collection = db.db.encounters
        return collection
    except Exception as e:
        logger.error(f"Error getting encounters collection: {str(e)}")
        return None
