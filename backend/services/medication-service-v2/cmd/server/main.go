...
func main() {
	...
	if cfg.GoogleFHIR.Enabled {
		...
		if err := googleFHIRClient.Initialize(ctx); err != nil {
			...
			// Continue without Google FHIR if initialization fails
	...
	// Initialize FHIR medication service for GraphQL with Google FHIR client
	...
	graphqlServer, err := http.NewGraphQLServer(medicationResolver, logger)
	if err != nil {
	...
	go func() {
		...
		// Start GraphQL server on a different port (8005)
		graphqlPort := "8005"
		logger.Info("Starting GraphQL Federation server", zap.String("port", graphqlPort))
