...
async def connect_to_mongo(max_retries: int = 3, retry_delay: int = 2) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Ensure the timeline_events collection exists
            await ensure_collection_exists("timeline_events")
            await ensure_collection_exists("timeline_cache")
...
def get_timeline_events_collection() -> Optional[motor.motor_asyncio.AsyncIOMotorCollection]:
    """Get the timeline_events collection."""
    if not db.is_connected():
        logger.warning("Database not connected, cannot get timeline_events collection")
        return None
    return db.db.timeline_events

def get_timeline_cache_collection() -> Optional[motor.motor_asyncio.AsyncIOMotorCollection]:
    """Get the timeline_cache collection."""
    if not db.is_connected():
        logger.warning("Database not connected, cannot get timeline_cache collection")
        return None
    return db.db.timeline_cache
