...
# Function to get current user from token (for medication service compatibility)
async def get_current_user_from_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
    """
    Get the current user from the token.

    This function validates the token with the Auth Service and returns the user information.
    It's designed for compatibility with the medication service.
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            logger.info(f"get_current_user_from_token: Calling Auth Service verify endpoint: {verify_url}")
            ...
            # Create a user object with the token and user info including RBAC details
            user = {
                "id": user_info.get("sub") or user_info.get("id", "unknown"),
            ...
            # Log RBAC information
            logger.info(f"User authenticated: {user['id']} with role: {role}")
            logger.info(f"User roles: {roles}")
            logger.info(f"User permissions: {permissions}")
