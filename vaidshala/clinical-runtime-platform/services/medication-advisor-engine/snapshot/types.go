// Package snapshot provides snapshot management for the Medication Advisor Engine.
// This package implements Calculate → Validate → Commit workflow with immutable snapshots.
package snapshot
...
// SnapshotType represents different types of clinical snapshots in the workflow
type SnapshotType string

const (
	// SnapshotTypeCalculation is for initial medication proposal generation
	SnapshotTypeCalculation SnapshotType = "calculation"
	// SnapshotTypeValidation is for validating selected proposal before commit
	SnapshotTypeValidation SnapshotType = "validation"
	// SnapshotTypeCommit is for final commit with FHIR MedicationRequest creation
	SnapshotTypeCommit SnapshotType = "commit"
	// SnapshotTypeMonitoring is for ongoing monitoring workflows
	SnapshotTypeMonitoring SnapshotType = "monitoring"
	// SnapshotTypeEmergency is for emergency medication decisions
...
type ClinicalSnapshot struct {
	...
	FreshnessMetadata  FreshnessMetadata    `json:"freshness_metadata"`
	...
	AuditTrail         []SnapshotAuditEntry `json:"audit_trail"`
}
...
// AssessmentScore represents clinical assessment scores (e.g., eGFR, SOFA)
...
const (
	...
	AssessmentRenal      AssessmentType = "renal"
	AssessmentHepatic    AssessmentType = "hepatic"
	AssessmentCardiac    AssessmentType = "cardiac"
)
...
const (
	...
	RiskCategoryRenal          RiskCategory = "renal"
	RiskCategoryHepatic        RiskCategory = "hepatic"
)
...
// ComputedScores contains pre-computed clinical scores
type ComputedScores struct {
	EGFR                          *float64 `json:"egfr,omitempty"`
	EGFRFormula                   string   `json:"egfr_formula,omitempty"`
	CKDStage                      string   `json:"ckd_stage,omitempty"`
	BMI                           *float64 `json:"bmi,omitempty"`
	BSA                           *float64 `json:"bsa,omitempty"`
	CreatinineClearance           *float64 `json:"creatinine_clearance,omitempty"`
	ChildPughScore                *int     `json:"child_pugh_score,omitempty"`
	ChildPughClass                string   `json:"child_pugh_class,omitempty"`
	RequiresRenalDoseAdjustment   bool     `json:"requires_renal_dose_adjustment"`
	RequiresHepaticDoseAdjustment bool     `json:"requires_hepatic_dose_adjustment"`
	FallRiskScore                 *float64 `json:"fall_risk_score,omitempty"`
	BleedingRiskScore             *float64 `json:"bleeding_risk_score,omitempty"`
}
