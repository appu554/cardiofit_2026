...
public class CIDAlert implements java.io.Serializable {
    ...
    public static CIDAlert create(CIDRuleId rule, String patientId,
        ...
        // Suppression key: ruleId + patientId + sorted medications hash
        String medsHash = medications == null ? "none"
            : String.valueOf(medications.stream().sorted()
                .reduce("", String::concat).hashCode());
...
public class ComorbidityState implements Serializable {
    ...
    private Double weightSevenDaysAgo;   // for weight delta computation
    private Double sbpSevenDayAvg;
    private Double fbgSevenDayAvg;       // for glucose trajectory
    private Double fbgFourteenDaysAgo;   // for FBG delta over 14 days
    private Double sbpFourteenDaysAgo;   // for SBP trajectory

    // --- Patient Demographics ---
    private Integer age;
    private Double latestGlucose;
    private boolean symptomReportedHypoglycemia;  // from patient-reported events
    private boolean symptomReportedMusclePain;    // for CID-08
    private boolean symptomReportedNauseaVomiting; // for CID-04, CID-09
    private boolean symptomReportedKetoDiet;       // for CID-04
    ...
    public Double getWeightDelta7d() {
        if (latestWeight == null || weightSevenDaysAgo == null) return null;
        return latestWeight - weightSevenDaysAgo;
    }
    ...
    public Double getFBGDelta14d() {
        if (fbgSevenDayAvg == null || fbgFourteenDaysAgo == null) return null;
        return fbgSevenDayAvg - fbgFourteenDaysAgo;
    }
    ...
    public Double getWeightSevenDaysAgo() { return weightSevenDaysAgo; }
    public void setWeightSevenDaysAgo(Double v) { this.weightSevenDaysAgo = v; }
    public Double getSbpSevenDayAvg() { return sbpSevenDayAvg; }
    public void setSbpSevenDayAvg(Double v) { this.sbpSevenDayAvg = v; }
    public Double getFbgSevenDayAvg() { return fbgSevenDayAvg; }
    public void setFbgSevenDayAvg(Double v) { this.fbgSevenDayAvg = v; }
    public Double getFbgFourteenDaysAgo() { return fbgFourteenDaysAgo; }
    public void setFbgFourteenDaysAgo(Double v) { this.fbgFourteenDaysAgo = v; }
    public Double getSbpFourteenDaysAgo() { return sbpFourteenDaysAgo; }
    public void setSbpFourteenDaysAgo(Double v) { this.sbpFourteenDaysAgo = v; }
...
public class Module8TestBuilder {
    ...
    /**
     * CID-01 Triple Whammy: ACEi + SGLT2i + thiazide + weight drop.
     * All three nephrotoxic pathways active + precipitant present.
     */
    public static ComorbidityState tripleWhammyPatient(String patientId) {
        ...
        state.setLatestWeight(72.0);
        state.setWeightSevenDaysAgo(75.0); // -3kg in 7 days
        state.setEGFRCurrent(48.0);
        state.setEGFRBaseline(60.0);
        state.setEGFRBaselineTimestamp(System.currentTimeMillis() - 90 * 86400000L);
    ...
    public static ComorbidityState tripleWhammyNoPrecipitant(String patientId) {
        ...
        state.addMedication("empagliflozin", "SGLT2I", 10.0);
        state.addMedication("chlorthalidone", "THIAZIDE", 12.5);
        state.setLatestWeight(74.0);
        state.setWeightSevenDaysAgo(74.5); // minimal weight change
        state.setEGFRCurrent(58.0);
        state.setEGFRBaseline(60.0); // <5% decline — normal
        return state;
    }
    ...
    public static ComorbidityState thiazideGlucosePatient(String patientId) {
        ...
        state.setFbgSevenDayAvg(128.0);
        state.setFbgFourteenDaysAgo(110.0); // delta = +18 mg/dL
        return state;
    }
    ...
    public static ComorbidityState glp1raGIPatient(String patientId) {
        ...
        state.setSymptomReportedNauseaVomiting(true);
        state.setLatestWeight(78.0);
        state.setWeightSevenDaysAgo(80.0); // -2kg in 7 days
        return state;
    }
    ...
    public static ComorbidityState concurrentDeteriorationPatient(String patientId) {
        ...
        state.setFbgSevenDayAvg(165.0);
        state.setFbgFourteenDaysAgo(135.0); // glucose worsening +30
        state.setSbpSevenDayAvg(152.0);
        state.setSbpFourteenDaysAgo(138.0); // BP worsening +14
        return state;
    }
    ...
    public static ComorbidityState safePatient(String patientId) {
        ...
        state.setFbgSevenDayAvg(112.0);
        state.setSbpSevenDayAvg(130.0);
        return state;
    }
}
```
...
class Module8HALTRulesTest {
    ...
    void cid01_tripleWhammy_fires() {
        ...
        List<CIDAlert> alerts = Module8HALTEvaluator.evaluate(state);
    ...
    void cid01_missingDiuretic_doesNotFire() {
        ...
        // No diuretic
        state.setLatestWeight(72.0);
        state.setWeightSevenDaysAgo(75.0);
        List<CIDAlert> alerts = Module8HALTEvaluator.evaluate(state);
        assertFalse(alerts.stream().anyMatch(a -> "CID_01".equals(a.getRuleId())),
...
public final class Module8HALTEvaluator {
     ...
     */
    private static void evaluateCID01(ComorbidityState state, List<CIDAlert> alerts) {
        ...
        Double weightDelta = state.getWeightDelta7d();
        if (weightDelta != null && weightDelta < -WEIGHT_DROP_THRESHOLD_KG) {
            weightDrop = true;
        }

        boolean eGFRDrop = false;
        Double eGFRDecline = state.getEGFRDeclinePercent();
        if (eGFRDecline != null && eGFRDecline >= EGFR_DROP_THRESHOLD_PCT - 1e-9) {
            eGFRDrop = true;
        }
    ...
    /**
     * CID-02: Hyperkalemia Cascade.
     * ACEi/ARB + finerenone + K+ > 5.3 rising.
     */
     ...
     */
    private static void evaluateCID05(ComorbidityState state, List<CIDAlert> alerts) {
        boolean hasSGLT2i = state.hasDrugClass("SGLT2I");
        ...
        Double sbpAvg = state.getSbpSevenDayAvg();
...
class Module8PAUSERulesTest {
    ...
    void cid06_thiazideGlucose_fires() {
        ...
        List<CIDAlert> alerts = Module8PAUSEEvaluator.evaluate(state);
    ...
    void cid06_thiazideSmallGlucoseChange_doesNotFire() {
        ...
        state.addMedication("hydrochlorothiazide", "THIAZIDE", 25.0);
        state.setFbgSevenDayAvg(118.0);
        state.setFbgFourteenDaysAgo(112.0); // delta = 6 — below 15 threshold
        List<CIDAlert> alerts = Module8PAUSEEvaluator.evaluate(state);
    ...
    void cid10_onlyGlucoseWorsening_doesNotFire() {
        ...
        state.setFbgSevenDayAvg(165.0);
        state.setFbgFourteenDaysAgo(135.0); // glucose worsening
        state.setSbpSevenDayAvg(132.0);
        state.setSbpFourteenDaysAgo(134.0); // BP stable/improving
        List<CIDAlert> alerts = Module8PAUSEEvaluator.evaluate(state);
...
public final class Module8PAUSEEvaluator {
    ...
    public static List<CIDAlert> evaluate(ComorbidityState state) {
        ...
        evaluateCID06(state, alerts);
        evaluateCID07(state, alerts);
        evaluateCID08(state, alerts);
        evaluateCID09(state, alerts);
        evaluateCID10(state, alerts);

        return alerts;
    }

    /** CID-06: Thiazide + FBG increase > 15 mg/dL in 14 days. */
    private static void evaluateCID06(ComorbidityState state, List<CIDAlert> alerts) {
        if (!state.hasDrugClass("THIAZIDE")) return;
        Double fbgDelta = state.getFBGDelta14d();
    ...
    /** CID-09: GLP-1RA + persistent GI + weight drop > 1.5 kg/7d. */
    private static void evaluateCID09(ComorbidityState state, List<CIDAlert> alerts) {
        if (!state.hasDrugClass("GLP1RA")) return;
        if (!state.isSymptomReportedNauseaVomiting()) return;
        Double weightDelta = state.getWeightDelta7d();
    ...
    /** CID-10: Concurrent glucose AND BP deterioration without medication change. */
    private static void evaluateCID10(ComorbidityState state, List<CIDAlert> alerts) {
        Double fbgDelta = state.getFBGDelta14d();
        boolean glucoseWorsening = fbgDelta != null && fbgDelta >= GLUCOSE_WORSENING_THRESHOLD - 1e-9;

        Double sbpDelta = null;
        if (state.getSbpSevenDayAvg() != null && state.getSbpFourteenDaysAgo() != null) {
            sbpDelta = state.getSbpSevenDayAvg() - state.getSbpFourteenDaysAgo();
        }
...
public class Module8_ComorbidityEngine
    ...
    public static final OutputTag<CIDAlert> HALT_TAG =
    ...
    public void processElement(CanonicalEvent event, Context ctx,
        ...
        List<CIDAlert> haltAlerts = Module8HALTEvaluator.evaluate(state);
        ...
        List<CIDAlert> pauseAlerts = Module8PAUSEEvaluator.evaluate(state);
    ...
    private void updateStateFromEvent(ComorbidityState state, CanonicalEvent event) {
        ...
        try {
            ...
            // Lab results
            if ("LAB_RESULT".equalsIgnoreCase(eventType)) {
                ...
                if (labType != null && value != null) {
                    ...
                    // Special handling for specific labs
                    if ("egfr".equalsIgnoreCase(labType)) {
                        if (state.getEGFRBaseline() == null) {
                            state.setEGFRBaseline(value);
                            state.setEGFRBaselineTimestamp(event.getEventTimestamp());
                        }
                        state.setEGFRCurrent(value);
                        state.setEGFRCurrentTimestamp(event.getEventTimestamp());
                    }
            ...
            // Vital signs
            if ("VITAL_SIGN".equalsIgnoreCase(eventType)) {
                ...
                if (sbp != null) state.setLatestSBP(sbp);
                if (dbp != null) state.setLatestDBP(dbp);
                if (weight != null) state.setLatestWeight(weight);
            }

            // Patient-reported symptoms
            if ("PATIENT_REPORTED".equalsIgnoreCase(eventType)) {
...
class Module8IntegrationTest {
    ...
    void safePatient_producesNoAlerts() {
        ...
        List<CIDAlert> halt = Module8HALTEvaluator.evaluate(state);
        List<CIDAlert> pause = Module8PAUSEEvaluator.evaluate(state);
    ...
    void suppressionPreventsRepeatedPAUSEAlerts() {
        ...
        List<CIDAlert> alerts1 = Module8PAUSEEvaluator.evaluate(state);
        ...
        List<CIDAlert> alerts2 = Module8PAUSEEvaluator.evaluate(state);
        for (CIDAlert a : alerts2) {
            if ("CID_08".equals(a.getRuleId())) {
                assertTrue(Module8SuppressionManager.shouldSuppress(a, state, now + 12 * 3600000L),
    ...
    void haltAndPauseCanCoexist() {
        // Patient triggers both CID-04 (HALT: SGLT2i + nausea) and CID-09 (PAUSE: GLP-1RA + GI)
        ComorbidityState state = new ComorbidityState("P-COEXIST");
        state.addMedication("dapagliflozin", "SGLT2I", 10.0);
        state.addMedication("semaglutide", "GLP1RA", 1.0);
        state.setSymptomReportedNauseaVomiting(true);
        state.setLatestWeight(78.0);
        state.setWeightSevenDaysAgo(80.0);
        ...
        assertTrue(pause.stream().anyMatch(a -> "CID_09".equals(a.getRuleId())),
            "CID-09 PAUSE should also fire (GLP-1RA + GI + weight drop)");
    }
...
Add launcher method (follows same pattern as `launchBPVariabilityEngine`).

- [ ] **Step 3: Run integration tests**

```bash
cd backend/shared-infrastructure/flink-processing && mvn test -pl . \
  -Dtest=Module8IntegrationTest -q 2>&1 | tail -15
```
