...
async function initializeGateway() {
  try {
    // Use IntrospectAndCompose with ONLY the services that are actually running
    ...
    const federationServices = [
      ...
      // Core services - DISABLED (start the service then uncomment)
      // { name: 'patients', url: 'http://localhost:8003/api/federation' },
      // { name: 'medications', url: 'http://localhost:8004/api/federation' },

      // Knowledge base services - DISABLED (uncomment when needed)
      ...
      // { name: 'kb4-patient-safety', url: 'http://localhost:8096/api/federation' },
      ...
      // Vaidshala Clinical Runtime services - DISABLED (uncomment when needed)
      // { name: 'kb20-patient-profile', url: 'http://localhost:8131/api/federation' },
      // { name: 'kb22-hpi-engine', url: 'http://localhost:8132/api/federation' },
      // { name: 'kb23-decision-cards', url: 'http://localhost:8134/api/federation' },
    ];
...
const startServer = async () => {
  try {
    ...
    // Set up CORS - removed global cors() to use specific CORS on GraphQL routes only
    // app.use(cors());
    ...
    // Evidence Envelope plugin disabled - service not running on port 8088
    ...
    const server = new ApolloServer({
      ...
      debug: true,
      // Evidence Envelope plugin disabled for now
    ...
    // Set up GraphQL endpoints with proper CORS for credentials
    const corsOptions = {
      origin: ['http://localhost:3000', 'http://localhost:3001', 'http://localhost:3002'],
      credentials: true
    };
    app.use('/graphql', cors(corsOptions), json(), graphqlMiddleware);
    // Add API Gateway compatible endpoint
    app.use('/api/graphql', cors(corsOptions), json(), graphqlMiddleware);
